...
function UnitDef_Post(name, uDef)
	...
	if uDef.extractsmetal and uDef.extractsmetal > 0 then
		uDef.customparams.metal_extractor = uDef.extractsmetal * 100
	end
	...
	if modOptions.unit_restrictions_noair and not uDef.customparams.ignore_noair then
		...
		if AircraftFactories[basename] then
			uDef.maxthisunit = 0
		end
	end
	...
	if uDef.israptor and uDef.health then

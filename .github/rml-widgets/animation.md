...
IMPORTANT: A transition fires on a class or pseudo-class change, an animation fires on element creation. Read the section `Animation` in SKILL.md before this. Upstream RmlUi behaviour unless a line says Measured.
...
- **Aggressive easing jitters.** Measured: `exponential-out`, `elastic-*`, and `bounce-*` cause visible sub-pixel jitter on small transforms such as `translateX(5dp)`. Prefer `quadratic-out` or `cubic-out` for subtle UI shifts.
